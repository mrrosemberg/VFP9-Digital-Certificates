...
IF NOT ecc.setencryptionlength(521)
   MESSAGEBOX('Error setting the encryption length. '+ecc.getLastError(),0+48,'Error')
   RETURN 
ENDIF    
* Sets the signing algorithm SHA-512(ECDSA)
IF NOT ecc.setSignAlg('sha512')
   MESSAGEBOX('Error setting the signing algorithm. '+ecc.getLastError(),0+48,'Error')
   RETURN 
ENDIF
* Set the certificate subject fields
ecc.CommonName = 'myCompany.local'
ecc.Email = 'admin@mycompany.local'
ecc.Locality = 'Rio de Janeiro'
ecc.State = 'RJ'
ecc.Organization = 'My Company'
ecc.OrganizationalUnit = 'My Company - Certification Authority'
ecc.Country = 'BR'
* Set the certificate validity
IF NOT ecc.setvalidity(730) && 2 years
   MESSAGEBOX('Error setting the certificate validity. '+ecc.getLastError(),0+48,'Error')
   RETURN 
ENDIF    
* Generate the CA Certificate
IF NOT ecc.generateCA()
   MESSAGEBOX('Error creating CA Certificate. '+ecc.getLastError(),0+48,'Error')
   RETURN
ENDIF
* Save the CA Certificate and Private key to a secure PKCS#12 file
IF NOT ecc.saveCaToPkcs12('ca.p12','Change-me$123')
   MESSAGEBOX('Error saving the CA Certificate. '+ecc.getLastError(),0+48,'Error')
   RETURN
ENDIF
* Export the CA Certificate. No need to export the private key
IF NOT ecc.savecaaspem('ca.cer')
   MESSAGEBOX('Error exporting the CA Certificate. '+ecc.getLastError(),0+48,'Error')
   RETURN
ENDIF
RELEASE ecc
*** All done
...
ecc.Country = 'BR'
* Set the certificate Alternative Names
*ecc.SanList = "email:"+ecc.Email+", Employee_Name.company.local"
*ecc.SanList = "email:"+ecc.Email+", Employee_Name.company.local, upn:emplyoee@mycompany.local, uri:urn:Employee_Name:123456"
